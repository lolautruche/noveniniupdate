...
class NovenClusterUpdater extends NovenConfigAbstractUpdater implements INovenFileUpdater
{
	const DEFAULT_INI_FILE = 'settings/override/file.ini.append.php',
	      DEFAULT_PHP_FILE = 'config.cluster.php',
	...
	private function updateDBFileHandler(SimpleXMLElement $clusterConf, $fileHandler)
	{
      ...
      switch((string)$clusterConf->DBBackend) {
        case 'eZDBFileHandlerOracleBackend':
          $storageBackend = 'oracle';
          break;
        case 'eZDBFileHandlerMysqliBackend':
          $storageBackend = 'mysqli';
          break;
        default:
	...
	private function updateDFSFileHandler(SimpleXMLElement $clusterConf)
	{
        ...
        try
        {
            ...
            switch((string)$clusterConf->DBBackend) {
              case 'eZDFSFileHandlerOracleBackend':
                $storageBackend = 'dfsoracle';
                break;
              case 'eZDFSFileHandlerMySQLiBackend':
                $storageBackend = 'dfsmysqli';
                break;
  	          default:
               $storageBackend = 'dfsmysql';
            }

            $phpGenerator->appendComment('Generated by NovenINIUpdate. '.date('Y-m-d H:i'));
            $phpGenerator->appendDefine('CLUSTER_STORAGE_BACKEND', $storageBackend);
            $phpGenerator->appendDefine('CLUSTER_STORAGE_HOST', $host);
            $phpGenerator->appendDefine('CLUSTER_STORAGE_PORT', $port);
            $phpGenerator->appendDefine('CLUSTER_STORAGE_SOCKET', (string)$clusterConf->DBSocket);
            $phpGenerator->appendDefine('CLUSTER_STORAGE_USER', (string)$clusterConf->DBUser);
            $phpGenerator->appendDefine('CLUSTER_STORAGE_PASS', (string)$clusterConf->DBPassword);
            $phpGenerator->appendDefine('CLUSTER_STORAGE_DB', (string)$clusterConf->DBName);
            $phpGenerator->appendDefine('CLUSTER_STORAGE_CHARSET','utf8');
			$phpGenerator->appendDefine('CLUSTER_MOUNT_POINT_PATH', (string)$clusterConf->MountPointPath);
            $phpGenerator->appendEmptyLines(1);
            //$phpGenerator->appendCustomCode("include('index_image.php');");
            $phpGenerator->finish();
        }
